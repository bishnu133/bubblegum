...
def test_ios_webview_switch_reporting_artifacts_are_safe(tmp_path) -> None:
    appium_webdriver = pytest.importorskip("appium.webdriver")
    options_module = pytest.importorskip("appium.options.ios")
    appium_url, values, has_app = _required_ios_webview_env()
    validate_text = os.getenv("BUBBLEGUM_IOS_WEBVIEW_VALIDATE_TEXT", "").strip()
    ...
    if not validate_text and not extract_ref:
        pytest.skip("Artifact validation needs operation metadata; set BUBBLEGUM_IOS_WEBVIEW_VALIDATE_TEXT and/or BUBBLEGUM_IOS_WEBVIEW_EXTRACT_REF.")
    options = options_module.XCUITestOptions()
    options.platform_name = "iOS"
    options.device_name = values["BUBBLEGUM_IOS_DEVICE_NAME"]
    options.automation_name = values["BUBBLEGUM_IOS_AUTOMATION_NAME"] or "XCUITest"
    ...
    try:
        ...
        step = StepResult(status="passed", action="iOS WebView artifact smoke", confidence=1.0, target=ResolvedTarget(ref="ios-webview://switch", confidence=1.0, resolver_name="ios_webview_smoke", metadata=safe_md))
        json_path = tmp_path / "ios_webview_switch_report.json"
        html_path = tmp_path / "ios_webview_switch_report.html"
        write_json_report([step], path=json_path, title="iOS WebView Switch Smoke Report")
        write_html_report([step], path=html_path, title="iOS WebView Switch Smoke Report")
