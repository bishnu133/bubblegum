...
def test_html_report_renders_system_dialog_section_when_present(tmp_path):
    ...
    assert "System Dialog Detection" in content
    assert "permission_prompt" in content


def test_html_report_hides_system_dialog_section_when_absent(tmp_path):
    out = tmp_path / "report.html"
    result = StepResult(status="passed", action="Tap", confidence=0.9)
    write_html_report([result], path=out)
    assert "System Dialog Detection" not in out.read_text(encoding="utf-8")


def test_html_report_escapes_system_dialog_diagnostic_values(tmp_path):
    out = tmp_path / "report.html"
    result = StepResult(
        status="passed",
        action="Tap",
        ...
        target=ResolvedTarget(
            ...
            metadata={"system_dialog_detection": {
                "dialog_detected": True,
                "dialog_type": "<script>alert(1)</script>",
                "platform": "android",
                "owner": "sys<tem>",
...
def test_html_report_renders_system_dialog_action_section_when_present(tmp_path):
    out = tmp_path / "report.html"
    result = StepResult(
        status="passed",
        action="Tap Allow",
        ...
        target=ResolvedTarget(
            ...
            metadata={"system_dialog_action": {
                ...
                "evidence": ["dialog:permission", "candidate:allow"],
                "warnings": ["metadata_only"],
    ...
    write_html_report([result], path=out)
    content = out.read_text(encoding="utf-8")
    assert "System Dialog Action" in content
    assert "executed" in content


def test_html_report_hides_system_dialog_action_section_when_absent(tmp_path):
    out = tmp_path / "report.html"
    write_html_report([StepResult(status="passed", action="Tap", confidence=0.9)], path=out)
    assert "System Dialog Action" not in out.read_text(encoding="utf-8")


def test_html_report_escapes_system_dialog_action_values(tmp_path):
    out = tmp_path / "report.html"
    result = StepResult(
        status="passed",
        action="Tap",
        ...
        target=ResolvedTarget(
            ...
            resolver_name="x",
            metadata={"system_dialog_action": {
                "action_status": "<script>alert(1)</script>",
                "reason": "unsafe<reason>",
                "warnings": ["<b>bad</b>"],
            }},
        ),
