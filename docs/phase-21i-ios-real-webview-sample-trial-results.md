# Phase 21I — iOS Real Sample WebView Trial Results

## 1. Purpose

This document captures the execution-ready trial plan and result template for validating iOS real-environment WebView switching on a sample app using the existing strict opt-in validate/extract smoke coverage from Phase 21C and reporting artifact safety checks from Phase 21D.
...
In scope:
- Real iOS sample-app run of the existing smoke test:
  - `test_ios_webview_switch_smoke_validate_extract_real_env`
- Real iOS sample-app run of the existing artifact safety test:
...
Use a real iOS sample app/screen that satisfies all of the following:
...
- The launch/install target is known (app path or bundle identifier).
...
export BUBBLEGUM_IOS_DEVICE_NAME=<placeholder>
# Use app path or bundle id:
export BUBBLEGUM_IOS_APP=<placeholder>
# export BUBBLEGUM_IOS_BUNDLE_ID=<placeholder>
export BUBBLEGUM_IOS_WEBVIEW_SWITCH_SMOKE=1
export BUBBLEGUM_IOS_WEBVIEW_VALIDATE_TEXT=<placeholder>
export BUBBLEGUM_IOS_WEBVIEW_EXTRACT_REF=<placeholder>
# Optional strict requirement:
# export BUBBLEGUM_IOS_WEBVIEW_REQUIRE_SWITCH=1
# Optional capabilities:
# export BUBBLEGUM_IOS_PLATFORM_VERSION=<placeholder>
# export BUBBLEGUM_IOS_AUTOMATION_NAME=XCUITest
...
- **Prepared, not executed** (real Appium + iOS simulator/device + sample app values were not supplied in this update).
...
2. Confirm iOS device/simulator name and capabilities match runtime.
3. Confirm sample app launch target (`IOS_APP` vs `IOS_BUNDLE_ID`) is correct.
4. Confirm target screen navigation and readiness timing.
5. Confirm `BUBBLEGUM_IOS_WEBVIEW_VALIDATE_TEXT` matches stable on-screen text.
6. Confirm `BUBBLEGUM_IOS_WEBVIEW_EXTRACT_REF` is valid and stable.
7. Re-run with optional `BUBBLEGUM_IOS_WEBVIEW_REQUIRE_SWITCH=1` only when switch enforcement is intentionally required.
8. Capture failure artifacts and map to existing 21C/21D expectations before proposing any code change.
...
1. Provision Appium endpoint, iOS simulator/device, and sample hybrid app/screen values.
2. Execute the two iOS trial commands exactly as listed in Section 5.
