...
## Unreleased
- Phase 12B docs/example release-readiness cleanup: fixed manual OpenAI vision example API usage to pass a BubblegumConfig object into configure_runtime (not a raw dict), removed unsupported `api_key=` argument from OpenAIVisionProvider construction, clarified that OPENAI_API_KEY is read by the OpenAI SDK environment, and reinforced `max_cost_level="high"` requirement for provider-based screenshot vision. Docs/checklist updates only; no runtime/API/adapter/resolver/dependency/version changes.
